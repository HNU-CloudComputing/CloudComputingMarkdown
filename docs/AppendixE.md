...
构造一个固定Tick循环，目标帧间隔16 ms。在不同的`cpu.max`配置下运行相同负载，记录每一帧真实间隔，并用CDF观察尾部劣化。进一步增加突发线程数，观察节流触发时机变化。

### 关键步骤

1.  编写固定Tick循环，逻辑开销尽量稳定，每帧记录开始与结束时间。

2.  用cgroups v2配置多组`cpu.max`，例如`100000 100000`、`50000 100000`、`20000 100000`。

3.  设计突发线程场景，例如主循环外增加2、4、8个短时CPU burst线程。

4.  每组运行至少5分钟，保存帧间隔序列。

5.  计算P50、P95、P99和最大值，并绘制帧间隔CDF。
...
在同构环境下分别部署三种CNI，使用`iperf3`测吞吐，使用自定义UDP ping测RTT分布，重点看平均值与P99稳定性。

### 关键步骤

1.  使用3个单节点集群（可用kind）保持CPU、内核与容器运行时一致。

2.  每个集群部署相同echo服务与相同资源请求限制。

3.  执行跨Pod网络测试，记录`iperf3`吞吐和UDP RTT序列。

4.  每组包大小至少测3档，例如64 B、512 B、1200 B。

5.  汇总平均RTT、P95、P99与抖动标准差。
...
1.  准备流量曲线，包含明显日周期和少量随机噪声。

2.  基于短窗口训练轻量LSTM，预测未来5到10分钟流量。

3.  策略A仅用HPA阈值触发，策略B根据预测提前扩容。

4.  在同一回放输入下记录P95延迟、SLO违约率和闲置资源比例。

5.  对比两策略在峰值前后时间段的行为差异。
...
1.  实现A调用B、B调用C的三层请求链，固定请求速率。

2.  在C层注入0.1%随机错误并写入Trace标签。

3.  设置1%、5%、10%头采样，以及错误优先的尾采样策略。

4.  多轮重复实验，统计发现首个错误Trace的平均时间与方差。

5.  估算各策略Trace写入量，绘制发现能力与成本关系图。
...
1.  部署3副本WebSocket echo服务，暴露统一访问入口。

2.  客户端维持长连接，每秒发送心跳并记录超时。

3.  注入PodKill故障，先测无PDB场景，再测有PDB场景。

4.  增加优雅下线配置（`preStop`钩子与连接排空），重复同样故障注入。

5.  统计断连时长分布、重连成功率与会话恢复时间。
...
加入`preStop`钩子与连接排空机制的更精细配置，比较中断分布尾部是否继续收敛，并分析对峰值期间容量的影响。
